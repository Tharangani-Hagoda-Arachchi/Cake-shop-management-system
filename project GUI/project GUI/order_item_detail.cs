        ...
        private void btnadd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string sql = @"INSERT INTO order_item_detail (pro_name,quantity,discount, idea,dil_date,order_id,pro_id)
                 VALUES ('" + txtproname.Text + "','" + Convert.ToDouble(txtquantity.Text) + "','" + Convert.ToDouble(txtdiscount.Text) + "','" + richTextBoxidea.Text + "','" + dateTimePickerdelivary.Text + "', '" + Convert.ToInt32(txtordid.Text) + "','" + txtproid.Text + "'); ";
                    MySqlCommand cmd = new MySqlCommand(sql, conn);//sql command object
                    cmd.ExecuteNonQuery();
                    conn.Close();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (MessageBox.Show("Are you want to Update?", "Update record", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    string sql = @"UPDATE order_item_detail SET pro_name = '" + txtproname.Text + "', quantity = '" + Convert.ToDouble(txtquantity.Text) + "',discount='" + Convert.ToDouble(txtdiscount.Text) + "',idea='" + richTextBoxidea.Text + "',dil_date='" + dateTimePickerdelivary.Text + "',order_id='" + Convert.ToInt32(txtordid.Text) + "',pro_id='" + txtproid.Text + "'  WHERE order_detail_id ='" + id + "';";
                    MySqlCommand cmd = new MySqlCommand(sql, conn);//sql command object
                    cmd.ExecuteNonQuery();
